...
for i in range(0, len(features)):
	feat = [features[i]['Age'],features[i]['Embarked'],features[i]['Fare'],features[i]['Sex'],float(features[i]['SiblingsAndSpouses'])+float(features[i]['ParentsAndChildren']), float(features[i]["Survived"])] #include survived in the features
	temprow.append(feat)
...
plt.figure(2)
dendrogram(Z, truncate_mode='lastp', p=6, leaf_rotation=50, leaf_font_size=8, show_contracted = True)
...
k = input("Enter the amount of clusters based on the dendrogram (for this particular dataset use 6: ")
k = np.int32(k)
